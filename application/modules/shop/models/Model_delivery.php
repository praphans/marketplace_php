...
class Model_delivery extends CI_Model {
  	...
  	public function getOrderTotalRows($store_id,$post)
	{
		...
		if(isset($order_place_id) && !empty($order_place_id))$this->db->where("store_place.request_place_id",$order_place_id);
		if(isset($order_status) && !empty($order_status))$this->db->where("order_status",$order_status);
	...
	{
		$query = $this->db->query('SELECT * FROM  store_place WHERE store_id = '.$store_id.' AND request_place_id IS NULL');
		if($query->num_rows() > 0 ) {
			return $query->result();
		} else {
			return array();
