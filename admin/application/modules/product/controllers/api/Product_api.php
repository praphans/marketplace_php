...
class Product_api extends MX_Controller {
	...
	{	
		...
		$product = $this->model_product->getProduct($current_store_id,$product_status_id);
        if(!empty($product)):
			foreach($product as $row):
				...
				$store_list = $this->model_product->getStoreByID($store_id);
				...
				if(strlen($store_name) > 60){
					$store_name = iconv_substr($store_name,0,40,"UTF-8")."...";
				}else{
					$store_name =  iconv_substr($store_name,0,40,"UTF-8");
				}

				$product_featured_result = $this->model_product->getProductFeatureByID($product_featured);
				...
				$status_name = "-";
				$status = $this->model_product->getProductStatusName($product_status);
				if(count($status))$status_name = $status[0]->status_name;

				$verify_url = base_url("product/verify/".$product_id."/".$store_id);
				$blocks_url = base_url("product/blocks/".$product_id."/".$store_id);
				$notblocks_url = base_url("product/notblocks/".$product_id."/".$store_id);
				if($product_status == 1){
				...
				$checkbox = '<input type="checkbox" id="category_'.$product_id.'" value="'.$product_id.'" class="filled-in chk-col-light-green chx_add_feature"><label for="category_'.$product_id.'" class="chk-mps"></label>';
				...
				$count_relate_result = $this->model_product->getCountRelateByID($product_id);
				...
				$sum_product_qty_result = $this->model_product->getProductQtyByID($product_id);
