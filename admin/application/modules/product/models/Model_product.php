...
class Model_product extends CI_Model {
    ...
    public function getProduct($store_id,$product_status_id){

        // if(!empty($product_status_id)){
        //     $status = ' AND product_status = '.$product_status_id;
        // }else{
        //     $status = '';
        // }
        // if(isset($store_id) && $store_id != 0){
        //     $query = $this->db->query('SELECT product_id,product_code,product_mode,product_name,product_qty,product_price,product_price_discount,product_featured,product_status,store_id FROM  product WHERE store_id = '.$store_id.' AND product_status != 5 AND relate_id = 0 '.$status);
        // }else{
        ...
        $query = $this->db->select('product_id,product_code,product_mode,product_name,product_qty,product_price,product_price_discount,product_featured,product_status,store_id');
        $this->db->from('product');
        
        if(!empty($store_id) && $store_id != 0){
           $this->db->where('store_id',$store_id);
        }
        if(!empty($product_status_id) && $product_status_id != 0){
           $this->db->where('product_status',$product_status_id);
        }
