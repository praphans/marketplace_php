                                                ...
                                                <ul class="list-group">
                                                    ...
                                                    <?php 
														function numMessage($message_id){
															$CI =& get_instance();
															$messages = $CI->model_message_inbox->getMessageANDReplyMessageByID($message_id);
															foreach($messages as $row){
																$message_id = $row->message_id;
																$message_reply_id = $row->message_reply_id;
															}
															if($message_reply_id > 0){
																$messages = $CI->model_message_inbox->getMessageANDReplyMessageByID($message_reply_id);
															
																$member_id = $CI->membermanager->member_id();
																
																$result = $CI->model_message_inbox->getMyReceiveMessage($member_id,$message_reply_id);
														...
														foreach($messages as $row){
															...
															if($filter == "receive"){
																//$messages_num = $this->model_message_inbox->getMessageANDReplyMessageByID($message_reply_id);
																...
																//$messages_num = $this->model_message_inbox->getMessageANDReplyMessageByID($message_id);
																$messages_num = numMessage($message_id);
																$last_sent = $this->model_message_inbox->getMyLastSenderMember($message_id);
																$last_sender_id = $last_sent[0]->sender_id;
																$last_time_send = $last_sent[0]->timestamp;
																//$last_message_id = $last_sent[0]->message_id;
															}
															...
															if($is_pin == 1){
																$active_pin = "active";
																$url_pin_unpin = base_url("message_inbox/unpin/".$message_id."/".$filter);
															}else{
																$active_pin = "";
																$url_pin_unpin = base_url("message_inbox/pin/".$message_id."/".$filter);
															}
															
															$member = $this->model_message_inbox->getMemberByID($last_sender_id);
														...
														?>
                                                        <a href="<?php echo base_url("message_inbox/info/".$filter."/".$message_type."/".$message_id); ?>" class="saveHome <?php echo $active; ?>">
                                                            <li class="list-group-item pt-10"><a class="<?php echo $active_pin; ?>" href="<?php echo $url_pin_unpin; ?>"><span class="mdi mdi-flag"></span></a> <a href="<?php echo base_url("message_inbox/info/".$filter."/".$message_type."/".$message_id); ?>" class="saveHome <?php echo $active; ?>"><?php echo $this->utils->string_shorten(strip_tags($message_topic),100,200); ?></a><p><?php echo $member_name." - ".$this->utils->time_ago($last_time_send); ?> <span class="mdi mdi-facebook-messenger"> </span><?php echo $messages_num; ?></p></li></a>
