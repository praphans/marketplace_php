...
class Message_inbox extends MX_Controller {

	public $PAGE;	
	public function __construct() {
        parent::__construct();
        $this->PAGE['title'] = 'ประวัติการสนทนา | '.$this->load->get_var("default_title");
		$this->load->model('message_inbox/model_message_inbox');
		$this->load->model('message_inbox/model_partner');
	...
	{
		$this->session->set_userdata("member_id_ref",$member_id);

		redirect("message_inbox/inbox/receive");
	}
	public function inbox($filter = "receive",$message_type = 0, $member_id = 0)
	{
		...
		$total_rows = $this->model_message_inbox->getMessageTotalRows($member_id,$filter,$message_type);
		$this->load->library('pagination');
		$config['base_url'] = base_url('message_inbox/inbox/'.$filter.'/'.$message_type.'/');
		...
		$this->PAGE['message_type_list'] = $this->model_message_inbox->getAllMessageType();
		if($filter == "sent"){
			$this->PAGE['messages'] = $messages = $this->model_message_inbox->getSenderMessage($member_id,$message_type,$page_number,$per_page);
		}else if($filter == "receive"){
			$this->PAGE['messages'] = $messages = $this->model_message_inbox->getReceiveMessage($member_id,$message_type,$page_number,$per_page);
		}
		//echo $total_rows."<br>";
		...
		$this->load->view("message_inbox/message_inbox_view",$this->PAGE);
	}
	// send_to 0 = ทุกคน
	...
	public function create($message_type_id = 0,$order_id = 0,$send_to = 0,$to_store_id = 0)
	{
		...
		$this->PAGE['message_type'] = $this->model_message_inbox->getMessageType();
		$this->load->view("message_inbox/create_view",$this->PAGE);
	}
	...
	public function reply()
	{
			...
			$messages = $this->model_message_inbox->getMessageByID($message_reply_id);
	...
	public function send()
	{
		...
		if(!isset($store_id) || $order_id != 0){
			if(isset($order_id) && $order_id != 0){	
				$order = $this->model_message_inbox->getOrderByID($order_id);
		...
		if(isset($store_id) && $send_to == 2){
			$store = $this->model_message_inbox->getMemberIDByStoreID($store_id); // ผู้ซื้อ
			if(count($store)){
		...
		if(isset($store_id) && ($send_to == 0 || $send_to == 1)){
			...
			$store = $this->model_message_inbox->getMemberIDByStoreID($store_id); // ผู้ขาย
		...
		if(isset($depositor_store_id) && ($send_to == 0 || $send_to == 3)){
			$depositor = $this->model_message_inbox->getMemberIDByStoreID($depositor_store_id);  // ผู้ส่ง
			if(count($depositor)){
		...
		if($send_to == 5){ // ส่งข่าวสาร ถึงผู้ติดตาม
			...
			$member_list = $this->model_message_inbox->getMyFollower($my_store_id);  
	...
	public function info($filter = "receive",$message_type = 0,$message_id = 0){
		...
		$messages = $this->model_message_inbox->getMessageANDReplyMessageByID($message_id);
		foreach($messages as $row){
			$message_id = $row->message_id;
			$message_reply_id = $row->message_reply_id;
		}
		if($message_reply_id > 0){
			$messages = $this->model_message_inbox->getMessageANDReplyMessageByID($message_reply_id);
		
			$member_id = $this->membermanager->member_id();
			
			$result = $this->model_message_inbox->getMyReceiveMessage($member_id,$message_reply_id);
		...
		$this->PAGE['message_type'] = $message_type;
		$this->PAGE['message_type_list'] = $this->model_message_inbox->getAllMessageType();
		$this->load->view("message_inbox/message_inbox_info_view",$this->PAGE);
	}
	public function pin($message_id,$filter){
		...
		redirect("message_inbox/inbox/".$filter."/0");
	}
