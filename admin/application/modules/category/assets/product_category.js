...
var categoryConfig = {
	category_url:config.base_url+"category/api/Category_product_api/getProduct",
	category_delete_url:config.base_url+"category/api/Category_product_api/deleteproduct",
...
function loadModalEdit(id){
	console.log(id);
	$(categoryConfig.category_container).empty();
	$(categoryConfig.category_container).load(config.base_url+"category/myModalEditproduct/"+id,function() {
...
function delCat(id,not_del){
	...
	if(hasItemSelected){
		swal({   
			...
			// alert(id);
			window.location = config.base_url+"category/delProductCatagory/"+id;
		});
